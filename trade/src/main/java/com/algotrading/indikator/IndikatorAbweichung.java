...
/**
 * ein Indikator als Kurs des selben Tages Parameter typ = open, close, high,
 * low, volume 1 = open 2 = close 3 = high 4 = low 5 = volume Parameter aktie =
 * Name der Aktie, die referenziert wird (optional) Parameter stabw = Anzahl
 * Tage für die Standardabweichung. Damit wird der Indikator ersetzt. Parameter
 * relativ = wenn vorhanden, dann: wie viele Standabweichungen ist der Kurs vom
...
public class IndikatorAbweichung extends IndikatorAlgorithmus {
	...
	public void rechne(Aktie aktie) {
		...
		if (oa != null) {
			aktie2 = aV.getVerzeichnis().getAktieMitKurse(oa.toString());
		} else {
			aktie2 = aktie;
		}
		...
		for (Kurs kurs : aktie.getKursListe()) { // der Kurs der Ziel-Aktie
			if (oa != null) { // wenn der Vergleichs-Kurs aus einer anderen Aktie stammt
				kurs2a = kurs2; // den Vortageskurs setzen
				kurs2 = aktie2.getKurs(kurs.getDatum()); // anhand des gleichen Datums den Vergleichs-Kurs holen
