...
public class Depot {
	...
	public void simuliereDepot(SignalStrategie signalStratgie, TagesStrategie tagesStrategie,
		...
		if (signalStratgie == null)
			log.error("Inputparameter Kaufstrategie = null");
		if (beginn == null)
			log.error("Inputparameter Beginn = null");
		if (ende == null)
			log.error("Inputparameter Ende = null");
		if (aktien == null || aktien.isEmpty())
			log.error("Inputparameter Aktien = null");
		if (!ende.after(beginn))
			log.error("Inputparameter Ende liegt vor Beginn");
	...
	private Kurs simuliereHandelstag(SignalStrategie kaufstrategie, TagesStrategie tagesStrategie) {
		...
		log.debug(
				"Handelstag: " + DateUtil.formatDate(this.heute) + " Signale: " + signale
						.size() + " Order: " + anzahlOrder + " Wert: " + kursDepot.close);
		return kursDepot;
	}
	...
	protected Order kaufe(float betrag, Aktie aktie) {
		if (aktie == null)
			log.error("Inputvariable Aktie ist null");
		Order result = null;
	...
	protected Order verkaufe(float betrag, Aktie aktie) {
		if (aktie == null)
			log.error("Inputvariable Kursreihe ist null");
		if (betrag == 0)
			log.error("Inputvariable betrag ist 0");
		Order order = null;
		...
		if (wertpapierbestand <= 0)
			log.error("Verkauf ohne Bestand");
	...
	boolean orderEintragen(Order order) {
		if (order == null)
			log.error("Inputvariable Order ist null");
	...
	private void addOrderToTrade(Order order) {
		...
		if (this.aktuelleTrades.containsKey(order.wertpapier)) {
			...
			if (order.kaufVerkauf == Order.VERKAUF)
				log.error("Verkauf in's Leere");
	...
	private String toStringHandelstagHeader() {
		String result = "Datum" + Util.separatorCSV + "Depotwert" + Util.separatorCSV;
		for (Aktie aktie : this.aktien) {
		...
		result = result.concat(
				"Signal1" + Util.separatorCSV + "Signal2" + Util.separatorCSV + "Signal3" + Util.getLineSeparator());
		return result;
	}
	...
	private BufferedWriter getHandelstagFile() {
		...
		if (fileWriterHandelstag == null) {
			...
			try {
				...
				log.error("File konnte nicht eröffnet werden: " + dateiname);
			}
